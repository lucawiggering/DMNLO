...
command  = './../../../dmnlo {} --slha ScenarioIa.spc  --legacy --renscheme 1 --particleA 1000006 --particleB -1000006 --particle1 25 --particle2 25 --pcm {} {}'
...
plt.plot(pcm,vsFull,color='red',label=r'$\sigma v_{\rm F}$')

pcmCheck, vsLOCheck = np.loadtxt('reference/Fig8a_vsLO.txt',unpack=True,delimiter=',')
plt.plot(pcmCheck,vsLOCheck,color='orange',label='LO pub',ls='dashed')
pcmCheck, vsNLOCheck = np.loadtxt('reference/Fig8a_vsNLO.txt',unpack=True,delimiter=',')
plt.plot(pcmCheck,vsNLOCheck,color='hotpink',label='NLO pub',ls='dashed')
pcmCheck, vsFullCheck = np.loadtxt('reference/Fig8a_vsFull.txt',unpack=True,delimiter=',')
plt.plot(pcmCheck,vsFullCheck,color='gray',label='Full pub',ls='dashed')

plt.title(r'$\tilde{t}_1 \tilde{t}^\ast_1 \to h^0 h^0$ legacy')
plt.xlabel(r'$p_{\rm cm}$ [GeV]')
plt.ylabel(r'$\sigma v$ [GeV$^{-2}$]')
plt.yscale('log')
plt.legend(frameon=False)
plt.savefig('Fig8a_legacy.pdf')