...
command  = './../../../dmnlo {} --slha ScenarioIa.spc  --legacy --renscheme 1 --particleA 1000006 --particleB -1000006 --particle1 23 --particle2 23 --pcm {} {}'
...
pcmCheck, vsLOCheck = np.loadtxt('reference/Fig8b_vsLO.txt',unpack=True,delimiter=',')
plt.plot(pcmCheck,vsLOCheck,color='orange',label='LO pub',alpha=0.6)
pcmCheck, vsNLOCheck = np.loadtxt('reference/Fig8b_vsNLO.txt',unpack=True,delimiter=',')
plt.plot(pcmCheck,vsNLOCheck,color='green',label='NLO pub',ls='dashed')
pcmCheck, vsFullCheck = np.loadtxt('reference/Fig8b_vsFull.txt',unpack=True,delimiter=',')
plt.plot(pcmCheck,vsFullCheck,color='gray',label='Full pub',ls='dashed')

plt.title(r'$\tilde{t}_1 \tilde{t}^\ast_1 \to Z^0 Z^0$ legacy')
plt.xlabel(r'$p_{\rm cm}$ [GeV]')
plt.ylabel(r'$\sigma v$ [GeV$^{-2}$]')
plt.yscale('log')
plt.legend(frameon=False)
plt.savefig('Fig8b_legacy.pdf')