...
command  = './../../../dmnlo {} --slha ScenarioIb.spc  --legacy --renscheme 1 --particleA 1000006 --particleB -1000006 --particle1 25 --particle2 35 --pcm {} {}'
...
pcmCheck, vsLOCheck = np.loadtxt('reference/Fig8c_vsLO.txt',unpack=True,delimiter=',')
plt.plot(pcmCheck,vsLOCheck,color='orange',label='LO pub',alpha=0.6)
pcmCheck, vsNLOCheck = np.loadtxt('reference/Fig8c_vsNLO.txt',unpack=True,delimiter=',')
plt.plot(pcmCheck,vsNLOCheck,color='green',label='NLO pub',ls='dashed')
pcmCheck, vsFullCheck = np.loadtxt('reference/Fig8c_vsFull.txt',unpack=True,delimiter=',')
