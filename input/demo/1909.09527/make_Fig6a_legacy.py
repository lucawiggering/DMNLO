...
command  = './../../../dmnlo {} --slha ScenarioI.spc --legacy --renscheme 1 --muR 3047.37 --particleA 1000015 --particleB -1000015 --particle1 6 --particle2 -6 --pcm {} {}'
...
pcmCheck, vsLOCheck = np.loadtxt('reference/Fig6a_vsLO.txt',unpack=True,delimiter=',')
plt.plot(pcmCheck,vsLOCheck,color='orange',label='LO pub',alpha=0.5)
pcmCheck, vsNLOCheck = np.loadtxt('reference/Fig6a_vsNLO.txt',unpack=True,delimiter=',')
plt.plot(pcmCheck,vsNLOCheck,color='gray',label='NLO pub',alpha=0.8,ls='dashed')
pcmCheck, vsFullCheck = np.loadtxt('reference/Fig6a_vsFull.txt',unpack=True,delimiter=',')
plt.plot(pcmCheck,vsFullCheck,color='hotpink',label='Full pub',alpha=0.8,ls='dashed')
pcmCheck, vsFullCheck = np.loadtxt('reference/Fig6a_vsSomm.txt',unpack=True,delimiter=',')
