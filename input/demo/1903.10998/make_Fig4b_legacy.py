...
command  = './../../../dmnlo {} --slha ScenarioII.spc --legacy --renscheme 1 --muR 2447.963 --particleA 1000005 --particleB 1000006 --particle1 5 --particle2 6 --pcm {} {}'
...
pcmCheck, vsLOCheck = np.loadtxt('reference/Fig4b_vsLO.txt',unpack=True,delimiter=',')
plt.plot(pcmCheck,vsLOCheck,color='orange',label='LO pub',alpha=0.5)
# pcmCheck, vsNLOCheck = np.loadtxt('reference/Fig11c_vsNLO.txt',unpack=True,delimiter=',')
# plt.plot(pcmCheck,vsNLOCheck,color='gray',label='NLO pub',alpha=0.8,ls='dashed')
# pcmCheck, vsFullCheck = np.loadtxt('reference/Fig11c_vsFull.txt',unpack=True,delimiter=',')
# plt.plot(pcmCheck,vsFullCheck,color='hotpink',label='Full pub',alpha=0.8,ls='dashed')

plt.title(r'$\tilde{t}_1 \tilde{b}_1 \to t b$ legacy')
plt.xlabel(r'$p_{\rm cm}$ [GeV]')
plt.ylabel(r'$\sigma v$ [GeV$^{-2}$]')
plt.yscale('log')
plt.ylim(4e-9,1e-7)
plt.legend()
plt.savefig('Fig4b_legacy.pdf')