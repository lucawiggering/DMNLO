...
extern void dd_dmnlo(int order, double muR, char *slha, int renscheme, int choosesol, int formfactor, double *cs);
...
void improveCrossSection(long n1,long n2,long n3,long n4 ,double PcmIn,  double * res) 
{
   ...
   corrFlag[0] = 1; // ChiChi2QQ:  gaugino (co-)annihilation into quarks, please cite 1404.2931
   ...
   corrFlag[5] = 1; // stsT2xx: stop-antistop annihilation into gluons and light quarks, please cite 2210.0526
...
// arguments are the SLHA input file as well as the name of the output file
...
// ----- DM@NLO settings -------------
   muR = computeQSUSY(); // choose renormalization scale to be the geometric mean of the stop masses
   // muR = 2*Mcdm; // choose the renormalization scale to be two times the dark matter mass
...
// ------------------------------------
   ...
   FILE *omega = fopen(argv[2],"w"); // open output file   
   fprintf(omega,"Block RESULTS\n");
...
// --- CalcHEP result ---
   ...
   fprintf(omega," %i %10.6e # Total relic density Omega h^2 with CalcHEP \n",1,Omega);

    // print (co)annihilation channels 
    w = 1.;
    i = 0;
    ...
    fprintf(omega," %i %6.6lf # DM@NLO total\n",200,w_dmnlo);
...
// --- DM@NLO tree-level result ---
   ...
   fprintf(omega," %i %10.6e # Total relic density Omega h^2 with DM@NLO @ LO \n",2,Omega);
...
// --- DM@NLO full result ---
   ...
   fprintf(omega," %i %10.6e # Total relic density Omega h^2 with DM@NLO @ NLO \n",3,Omega);
...
/// --- MicrOMEGAs ---
   ...
   fprintf(omega," 41 %8.6f # Direct detection pval CalcHEP\n",pval);
   fprintf(omega," 411 %10.6e # proton SI [cm^2]\n",csSIp*pb2cm2);
   fprintf(omega," 412 %10.6e # proton SD [cm^2]\n",csSDp*pb2cm2);
   fprintf(omega," 413 %10.6e # neutron SI [cm^2]\n",csSIn*pb2cm2);
   fprintf(omega," 414 %10.6e # neutron SD [cm^2]\n",csSDn*pb2cm2);
...
/// --- DM@NLO tree-level ---
   ...
   fprintf(omega," 42 %8.6f # Direct detection pval DM@NLO @ LO\n",pval);   
   fprintf(omega," 421 %10.6e # proton SI [cm^2]\n",ddres[0]);
   fprintf(omega," 422 %10.6e # proton SD [cm^2]\n",ddres[1]);
   fprintf(omega," 423 %10.6e # neutron SI [cm^2]\n",ddres[2]);
   fprintf(omega," 424 %10.6e # neutron SD [cm^2]\n",ddres[3]);  
...
/// --- DM@NLO NLO ---
   ...
   fprintf(omega," 43 %8.6f # Direct detection pval DM@NLO @ NLO\n",pval);
   fprintf(omega," 431 %10.6e # proton SI [cm^2]\n",ddres[0]);
   fprintf(omega," 432 %10.6e # proton SD [cm^2]\n",ddres[1]);
   fprintf(omega," 433 %10.6e # neutron SI [cm^2]\n",ddres[2]);
   fprintf(omega," 434 %10.6e # neutron SD [cm^2]\n",ddres[3]);   
        
#endif
  
   fclose(omega); // close output file
